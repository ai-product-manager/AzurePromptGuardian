...
async def analyze_text_features(text: str) -> dict:
    try:
        ...
        language = await asyncio.to_thread(client.detect_language, documents=[text])
        
        # Nuevo: análisis de errores gramaticales
        ...
        return {
            ...
            "language": language[0].primary_language.name,
            "grammar_quality": {
                "opinion_mining_present": len(sentiment[0].sentences) > 0 and hasattr(sentiment[0].sentences[0], 'mined_opinions')
            }
        }
    except Exception as e:
...
async def analyze_prompt(request: PromptRequest, background_tasks: BackgroundTasks):
    ...
    try:
        ...
        # Análisis de texto y características lingüísticas
        text_results = asyncio.create_task(analyze_text_features(clean_prompt))
        
        # Nuevo: Análisis de ambigüedad y claridad
        ambiguity_results = asyncio.create_task(analyze_ambiguity(clean_prompt))
        
        text_results, ambiguity_results = await asyncio.gather(
            text_results, ambiguity_results
        )
        
        # Redactar PII
        redacted_prompt = redact_pii(clean_prompt, text_results["pii"])
        
        # Análisis completo con OpenAI para mejorar el prompt
        openai_client = get_openai_client()
        ...
        # Calcular puntuación de equidad basada también en el análisis de texto
        calculated_fairness = calculate_fairness_bias_score(text_results, safety_results)
        adjusted_fairness = (analysis_data["fairness_score"] + calculated_fairness) / 2
        
        # Generar variantes del prompt si se solicitó
        prompt_variants = []
        if request.generate_variants:
            prompt_variants = await generate_prompt_variants(
                clean_prompt, 
                request.context, 
                request.optimization_focus
            )
            
            # Agregar tarea en segundo plano para mejorar los modelos internos
            # basados en las variantes generadas
            background_tasks.add_task(
                log_prompt_variants_for_learning,
                clean_prompt,
                [v.variant_text for v in prompt_variants]
            )
        
        # Preparar lista de problemas detectados
        issues = []
        
        # Añadir problemas de ambigüedad
        if ambiguity_results.get("ambiguity_score", 0) > AMBIGUITY_THRESHOLD:
            issues.append(AnalysisIssue(
                type="ambiguity",
                description="Prompt con alto nivel de ambigüedad",
                severity="medium" if ambiguity_results.get("ambiguity_score", 0) > 0.8 else "low",
                mitigation="Considera especificar los términos ambiguos y proveer contexto adicional"
            ))
        
        # Añadir problemas de completitud
        ...
        # Añadir problemas basados en análisis de OpenAI
        ...
        audit_doc = {
            ...
            "variants": [v.dict() for v in prompt_variants] if prompt_variants else []
        }
        ...
        return AnalysisResponse(
            ...
            accountability_id=analysis_id,
            suggested_variants=prompt_variants if prompt_variants else None,
            improvement_explanation=analysis_data.get("improvement_explanation", ""),
            issues=issues
        )
